...
export const Dropdown = (props: TDropdownProps) => {
  const { className, content, items, position = 'bottom-right' } = props;

  return (
    <Menu as="div" className={getClassName(cls.dropdown, {}, [className])}>
      <Menu.Button>{content}</Menu.Button>
      <Menu.Items
        className={getClassName(cls.items, {}, [mapDirectionClasses[position]])}
      >
        {items.map((item, i) => (
          <Menu.Item
            as="div"
          ...
          >
            {item.href ? (
              <AppLink className={getClassName(cls.link)} to={item.href}>
                {item.content}
              </AppLink>
            ) : (
              <Button
